			...
			jQuery.getJSON(vmSiteurl+"index.php?option=com_virtuemart&nosef=1&view=cart&task=viewJS&format=json"+vmLang,
				function(datas, textStatus) {
					if (datas.totalProduct >0) {
						...
						mod.find(".vm_cart_products").html("").css('height','auto');
						mod.find(".total").html("");
						mod.find(".tot3").html("");
						mod.find(".tot4").html("");
						mod.find(".total2").html(datas.billTotal);
		...
		var Virtuemart = {
			...
			cartEffect : function(form) {
					...
					function(datas, textStatus) {
						...
						var this_prod = form.find(".item_id").val() ;
						var img_url = document.getElementById('Img_to_Js_'+this_prod).src;
                        var txt = "<H4>"+form.find(".pname").val()+"</H4>"+datas.msg+"<img width='80' height='auto' src='"+img_url+"' />";
                    } else {
...
(function($){
	...
	methods = {
		...
		update: function(org,dest,ids,prefix) {
			...
			if (byAjax.length >0) {
				$.getJSON('index7c57.html?option=com_virtuemart&amp;view=state&amp;format=json&amp;virtuemart_country_id=' + byAjax,
						function(result){
